...
class TransformerTrainer(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.begin_steps = begin_steps
        self.register_buffer('steps', torch.Tensor([begin_steps]))
        ...
        self.max_save_model = max_save_model
        self.lr=lr
        # Load the pre-trained weights
        self.ds = VideoTextDataset(data_folder=data_folder, xlsx_file=train_csv_folder, num_frames=2)
        ...
        self.valid_ds = VideoTextDataset(data_folder=data_folder, xlsx_file=val_csv_folder, num_frames=2)
        # self.ds,_ = random_split(self.ds, [1.0, 0.0], generator = torch.Generator().manual_seed(42))
        # self.valid_ds,_ = random_split(self.valid_ds, [1.0, 0.0], generator = torch.Generator().manual_seed(42))
        # list_train=[]
        # list_val=[]
        ...
        # with open(log_folder + "train_transformer.txt", "w") as f:
        #     for item in list_train:
        #         f.write(str(item) + "\n")
        # with open(log_folder + "valid_transformer.txt", "w") as f:
        ...
        if len([*self.results_folder.glob('**/*')]) > 0 and remove_checkpoint:
            # rmtree(str(self.results_folder))
            for pt_file in self.results_folder.glob('*.pt'):
                pt_file.unlink()
    ...
    def train_step(self):
        ...
        if self.is_main and not ((steps + 1) % self.save_results_every):
            with torch.no_grad():
                ...
                for model, filename in vaes_to_evaluate:
                    ...
                    recons = model.sample(texts=text, num_frames=129, cond_scale=5.)
                    ...
                    for tensor in recons.unbind(dim=0):
                        ...
                        valid_data_cpu = valid_data[0].cpu()
                        tensor_to_nifti(tensor_cpu, str(sampled_videos_path / f'{filename}_{i}.nii.gz'))
                        tensor_to_nifti(valid_data_cpu, str(sampled_videos_path / f'{filename}_{i}_input.nii.gz'))
                        i += 1
                        del tensor_cpu, valid_data_cpu
                    
                    with open(str(sampled_videos_path / f'{filename}_{i}.txt'), "w", encoding="utf-8") as file:
                        file.write(text[0])
                    
                    del recons, valid_data
                    torch.cuda.empty_cache()
                
                self.print(f'{steps}: saving to {str(self.results_folder)}')

        if self.is_main and not ((steps + 1) % self.save_model_every):
            ...
            files = sorted(self.results_folder.glob('maskgittransformer.*.pt'), key=lambda x: x.stat().st_mtime)
            if len(files) > self.max_save_model:
                for old_file in files[:-self.max_save_model]:
                    old_file.unlink()  
    ...
    def train(self, log_fn=noop):
        ...
        pbar = tqdm.tqdm(total=(self.num_train_steps-self.begin_steps), desc="Training")

        while self.steps < (self.num_train_steps-self.begin_steps):
