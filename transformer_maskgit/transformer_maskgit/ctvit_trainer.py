...
class CTViTTrainer(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.discr_optim = get_optimizer(discr_parameters, lr = 0.01*lr, wd = wd)
        ...
        with open(log_folder + "train.txt", "w") as f:
            for item in list_train:
                f.write(str(item) + "\n")
        with open(log_folder + "valid.txt", "w") as f:
    ...
    def train(self, log_fn=noop):
        ...
        # 初始化 tqdm 进度条，tqdm 会自动计算 ETA
        pbar = tqdm.tqdm(total=self.num_train_steps, desc="Training")
