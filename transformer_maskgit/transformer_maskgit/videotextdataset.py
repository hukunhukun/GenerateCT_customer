...
class VideoTextDataset(Dataset):
    def __init__(self, data_folder, xlsx_file, min_slices=20, resize_dim=128, num_frames=2, force_num_frames=True):
        ...
        if xlsx_file.endswith('.xlsx'):
            ...
            self.accession_to_text = self.load_accession_text_csv(xlsx_file)
            self.samples = self.prepare_samples_csv()
        else:
            raise ValueError("Provided file is not valid")
    ...
    def load_accession_text_csv(self, csv_file):
        df = pd.read_csv(csv_file)
        accession_to_text = {}
        for index, row in df.iterrows():
            accession_to_text[row['image']] = row['text']
        return accession_to_text
    ...
    def prepare_samples_csv(self):
        samples = []
        
        for nii_file in glob.glob(os.path.join(self.data_folder, '**', '*.nii.gz'), recursive=True):
            basename = os.path.basename(nii_file)  # 示例：left_hip_s0010.nii.gz
            
            if basename not in self.accession_to_text:
                continue
                
            text = self.accession_to_text[basename]
    ...
    def nii_img_to_tensor(self, path, transform):
        ...
        tensor = torch.stack(slices,dim=1)
        tensor = tensor.unsqueeze(1)
        tensor=F.interpolate(tensor, size=(129, 128, 128), mode='trilinear',align_corners=False)
        tensor = tensor.squeeze(1)
        return tensor
