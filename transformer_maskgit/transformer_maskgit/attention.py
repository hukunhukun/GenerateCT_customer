...
class ContinuousPositionBias(nn.Module):
    ...
    def forward(self, *dimensions, device = torch.device('cpu')):
        ...
        if not hasattr(self, 'rel_pos') or not self.cache_rel_pos:
            positions = [torch.arange(d, device=device) for d in dimensions]
            grid = torch.stack(torch.meshgrid(*positions, indexing='ij'))
            grid = rearrange(grid, 'c ... -> (...) c')
            rel_pos = rearrange(grid, 'i c -> i 1 c') - rearrange(grid, 'j c -> 1 j c')
        ...
        rel_pos = self.rel_pos.to(torch.float32)
        rel_pos = checkpoint_sequential(self.net, segments=len(self.net), input=rel_pos)
...
# def transformer_layer_forward(x, peg, self_attn, cross_attn, ff,
#                               video_shape, attn_bias, self_attn_mask,
#                               context, cross_attn_context_mask):
#     if exists(peg):
#         x = peg(x, shape=video_shape) + x
#     x = self_attn(x, attn_bias=attn_bias, mask=self_attn_mask) + x
#     if exists(cross_attn) and exists(context):
